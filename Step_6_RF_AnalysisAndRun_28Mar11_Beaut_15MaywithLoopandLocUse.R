...
setwd("C:/Users/Public/OPRHP_Nat_Heritage_Ranks")
...
#code_name<-gsub("-01","",as.character(att_pres_points[[i]]$siteID[1]))
code_name<-sapply(strsplit(as.character(att_pres_points[[i]]$siteID[1]),"-"),"[",1) #grab code from name in att_pres_points list
...
for (elem.i in 1:length(elems)){
  ...
  ##  open up a "channel" to the Access DB to lookup common name and code and Location Use Class
  ##Keep in mind that elements in the list "att_pres_points" are named by their code- so can look up by codetes
	...
	ElementNames <- as.list(c(SciName="",CommName="",Code="",Type="",Loc_Use=""))##Changed this to add Loc_Use Class
	ElementNames[1] <- as.character(df.in.ss[1,"sci_name"])
  ElementNames[3]<- as.character(code_name)
...
# 	ElementNames[3] <- as.list(dbGetQuery(db, statement = SQLquery)[1,1])
  ...
  SQLquery <- paste("SELECT Loc_Use FROM tblInputs WHERE Code = '", ElementNames[3],"';", sep="")
  ElementNames[5]<- as.list(dbGetQuery(db, statement = SQLquery)[1,1])
  ElementNames
	#
	#close(Cn.MDB.in.i)
	#rm(Cn.MDB.in.i)
	...
	cat("starting on",ElementNames[[1]],ElementNames[[5]],"\n")##AKC Changed this to add Loc_Use
  ...
  ##get the mean cutoff values for the validation runs
	...
	OutPut <- data.frame(SciName = as.character(ElementNames$SciName),
               ...
               Loc_Use=as.character(ElementNames$Loc_Use)
	             )
  ...
  setwd("C:/Users/Public/2015Pros/RdataFiles")
	save.image(file = paste(ElementNames$SciName," ",ElementNames$Loc_Use, ".Rdata", sep=""))
	...
	cat("done with", ElementNames[[1]],ElementNames[[5]], "\n")
